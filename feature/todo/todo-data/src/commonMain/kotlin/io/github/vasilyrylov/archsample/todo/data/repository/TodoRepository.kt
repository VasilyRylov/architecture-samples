...
class TodoRepository(
    ...
    override suspend fun save(todoItem: TodoItem, userId: UserId) {
        return withContext(Dispatchers.IO) {
            database.getTodoDao()
                .save(TodoMapper.toDatabase(todoItem, userId.value.toString()))
        }
    }
    ...
    override suspend fun getById(todoItemId: TodoItemId): TodoItem {
        return withContext(Dispatchers.IO) {
            database.getTodoDao()
                .getTodoById(todoItemId.value.toString()).let { TodoMapper.fromDatabase(it) }
        }
    }

    override suspend fun delete(todoItemId: TodoItemId) {
        return withContext(Dispatchers.IO) {
            database.getTodoDao().delete(todoItemId.value.toString())
        }
    }
}
